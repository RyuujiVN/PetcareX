...
const MessageBox = () => {
  const { t } = useTranslation(["vererianrian", "client"]);
  ...
  const handleSelectImages = async (event) => {
    ...
    if (invalidFile) {
      message.warning(t("pages.chatbot.messageBox.validation.imageOnly"));
      event.target.value = "";
      return;
    }
    ...
    try {
      ...
      message.error(error?.message || t("pages.chatbot.messageBox.uploadFailed"));
    } finally {
  ...
  useEffect(() => {
    ...
    const onServerResponseError = (errorData) => {
      const errMsg =
        errorData?.message || t("pages.chatbot.messageBox.genericError");
      ...
      <div className="messages-container" ref={messagesContainerRef}>
        {!roomId ? (
          <div className="empty-state">
            <h2>{t("pages.chatbot.messageBox.emptyState")}</h2>
          </div>
        ) : (
          <>
            {isLoadingMore && (
              ...
              >
                <Spin size="middle" />
                <span>{t("pages.chatbot.messageBox.loadingOldMessages")}</span>
              </div>
            )}
      ...
      <div className="chatbot-input-dock">
            ...
            <div className="inline-image-preview-list">
              ...
              >
                <img
                  ...
                  alt={t("pages.chatbot.messageBox.previewAlt")}
                ...
                <button
                  ...
                  aria-label={t("pages.chatbot.messageBox.removeImage")}
        ...
        >
          <button
            ...
            title={t("pages.chatbot.messageBox.attachImage")}
            aria-label={t("pages.chatbot.messageBox.attachImage")}
            disabled={isAiLoading || isUploadingImages}
          >
            <Plus size={20} />
          </button>
          ...
          <Form.Item name="content" style={{ flex: 1, marginBottom: 0 }}>
            <Input
              ref={inputRef}
              placeholder={t("pages.chatbot.messageBox.inputPlaceholder")}
          ...
          <Form.Item style={{ marginBottom: 0 }}>
            {isAiLoading ? (
              <button
                ...
                className="send-btn"
                style={{ padding: 0 }}
                title={t("pages.chatbot.messageBox.aiResponding")}
              ...
              <button
                ...
                title={
                  isUploadingImages
                    ? t("pages.chatbot.messageBox.uploadingImage")
                    : t("pages.chatbot.messageBox.send")
                }
              >
                <Send size={20} />
              </button>
        ...
        <p className="footer-info">
          {isUploadingImages
            ? t("pages.chatbot.messageBox.uploadingImageLong")
            : t("pages.chatbot.messageBox.disclaimer")}
        </p>
      </div>
    </div>
  );
